...
public class AddProductController {
   ...
   @FXML
    ...
    /**
     * saves the product if it validates all the textboxes information
     * @param event
     */
    @FXML
    void saveNewProduct(MouseEvent event)
    {
        ...
        {
            ShowMessageBox("Error in validation","Make sure the correct information is in the name box,\n your name should be in that box", Alert.AlertType.ERROR);
            return;
        }
        if(!checkInteger(txtInv.getText()) || checkEmpty(txtInv.getText()))
        {
            ShowMessageBox("Error in validation","Make sure the correct information is in the inventory box,\n the value should be a number", Alert.AlertType.ERROR);
            return;
        }
        if(!checkDouble(txtPrice.getText())|| checkEmpty(txtPrice.getText()))
        {
            ShowMessageBox("Error in validation","Make sure the correct information is in the price box,\n the value should be a number", Alert.AlertType.ERROR);
            return;
        }
        if(!checkInteger(txtMax.getText())|| checkEmpty(txtMax.getText()))
        {
            ShowMessageBox("Error in validation","Make sure the correct information is in the max box,\n the value should be a number", Alert.AlertType.ERROR);
            return;
        }
        if(!checkInteger(txtMin.getText())|| checkEmpty(txtMin.getText()))
        {
            ShowMessageBox("Error in validation","Make sure the correct information is in the min box,\n the value should be a number", Alert.AlertType.ERROR);
            return;
        }
        ...
        {
            ShowMessageBox("Error in validation","Make sure max is greater than the min and both\n of them greater than 0.", Alert.AlertType.ERROR);
            return;
        }
        if(inv > max || inv < min)
        {
    ...
    /**
     * generates a new id using the size of the allProducts observableList
     * @return
     */
    int getIdMaxNumber()
    {
        return Inventory.getAllProducts().size();
    }
    /**
